            ...
            ` : `
              ...
              ${activeSheetTab === 'select' ? `
                ...
                <div style="margin-bottom: 10px; display: flex; flex-direction: column; gap: 8px;">
                  <input type="text" id="partner-search-input" placeholder="Search partner name or ID..." value="${searchQuery}" style="width: 100%; padding: 8px 12px; font-size: 0.82rem; border-radius: 8px; border: 1px solid #ede4da; background: #fbf8f4; outline: none; box-sizing: border-box;">
...
export function attachAndroidLoginEvents(onLoginSuccess) {
  ...
  // Send OTP
  document.getElementById('btn-sheet-send-otp')?.addEventListener('click', () => {
    const input = document.getElementById('auth-sheet-input');
  ...
  // Verify OTP
  document.getElementById('btn-sheet-verify-otp')?.addEventListener('click', () => {
