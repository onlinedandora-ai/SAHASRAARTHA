/**
 * iOS & Mobile Reference-Matching Authentication Screen
  ...
  return `
    <div class="auth-screen-container">
      ...
      <div class="auth-button-stack">
        ...
        <!-- 1. Super Admin Login -->
        <button class="auth-pill-btn-orange" id="btn-super-admin-action-ios">
        ...
        <!-- 2. Login with Phone -->
        <button class="auth-pill-btn-outline" id="btn-login-phone-action-ios">
          <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <!-- 3. Login with Email -->
        <button class="auth-pill-btn-outline" id="btn-login-email-action-ios">
          <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <button class="auth-pill-btn-blue" id="btn-continue-google-action-ios">
        ...
        <button class="auth-pill-btn-dark" id="btn-continue-apple-action-ios">
...
function renderAuthModalSheetIOS() {
      ...
      <div class="auth-sheet-backdrop" id="sheet-backdrop-ios">
        <div class="auth-sheet-dialog">
          ...
          <div style="display: flex; flex-direction: column; gap: 14px;">
            ...
            <button class="auth-pill-btn-orange" id="btn-admin-instant-bypass-ios" style="font-weight: 800; padding: 14px; font-size: 0.95rem;">
          ...
          <div style="display: flex; flex-direction: column; gap: 14px;">
            <div class="form-group">
              ...
              <div style="display: flex; gap: 8px;">
                <input type="text" id="input-phone-number-ios" class="form-input" placeholder="e.g. 9821837797, Srikanth, Panathula" value="${pendingPhoneValue}" style="flex: 1; padding: 12px 14px; font-size: 0.92rem; border-radius: 12px; border: 1.5px solid var(--border-color); background: var(--bg-card); color: var(--text-primary); outline: none;">
              </div>
            </div>

            <button class="auth-pill-btn-orange" id="btn-send-phone-otp-ios" style="font-weight: 800;">
...
export function attachIOSLoginEvents(onLoginSuccess) {
  ...
  document.getElementById('btn-super-admin-action-ios')?.addEventListener('click', () => {
  ...
  document.getElementById('btn-open-whitelist-action-ios')?.addEventListener('click', openPartnerWhitelist);
  document.getElementById('link-partner-whitelist-btn-ios')?.addEventListener('click', openPartnerWhitelist);
  document.getElementById('btn-view-all-partners-from-phone-ios')?.addEventListener('click', openPartnerWhitelist);
  document.getElementById('btn-view-all-partners-from-email-ios')?.addEventListener('click', openPartnerWhitelist);

  // 3. Direct Phone Login Action
  document.getElementById('btn-login-phone-action-ios')?.addEventListener('click', () => {
  ...
  document.getElementById('btn-login-email-action-ios')?.addEventListener('click', () => {
  ...
  document.getElementById('btn-continue-google-action-ios')?.addEventListener('click', async () => {
    const btn = document.getElementById('btn-continue-google-action-ios');
    ...
    try {
      ...
      console.warn('[IOSLogin] Google sign-in fallback to direct login:', err);
      const admin = store.partners.find(p => p.partnerId === 'SH-SA-001') || store.partners[0];
      executeDirectLogin(admin, onLoginSuccess);
    }
  });
  ...
  document.getElementById('btn-continue-apple-action-ios')?.addEventListener('click', () => {
    const admin = store.partners.find(p => p.partnerId === 'SH-SA-001') || store.partners[0];
    if (admin) {
      executeDirectLogin(admin, onLoginSuccess);
    }
  ...
  const searchInput = document.getElementById('direct-search-input-ios');
  if (searchInput) {
    searchInput.addEventListener('input', (e) => {
      searchQuery = e.target.value;
      store.notify();
      setTimeout(() => {
        const renewedInput = document.getElementById('direct-search-input-ios');
        if (renewedInput) {
          renewedInput.focus();
  ...
  // Close Sheet
  document.getElementById('btn-close-sheet-ios')?.addEventListener('click', () => {
    activeSheet = null;
    store.notify();
  });

  document.getElementById('sheet-backdrop-ios')?.addEventListener('click', (e) => {
    if (e.target.id === 'sheet-backdrop-ios') {
      activeSheet = null;
      store.notify();
    }
  });
  ...
  document.getElementById('btn-admin-instant-bypass-ios')?.addEventListener('click', () => {
    const admin = store.partners.find(p => p.partnerId === 'SH-SA-001') || store.partners[0];
    executeDirectLogin(admin, onLoginSuccess);
  });

  // Phone Direct Flow (Zero OTPs)
  document.getElementById('btn-send-phone-otp-ios')?.addEventListener('click', () => {
    const phoneInput = document.getElementById('input-phone-number-ios')?.value.trim();
    const matched = (phoneInput ? findPartnerByIdentifier(phoneInput) : null) || store.partners[0];
    executeDirectLogin(matched, onLoginSuccess);
  });

  // Email Direct Flow (Zero OTPs)
  document.getElementById('btn-send-email-otp-ios')?.addEventListener('click', () => {
    const emailInput = document.getElementById('input-email-address-ios')?.value.trim();
